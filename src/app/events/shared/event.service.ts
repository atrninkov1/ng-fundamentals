...
export class EventService{
    ...
    searchSessions(searchTerm: string){
      ...
      EVENTS.forEach(event => {var matchingSession = event.sessions.filter(session => session.name.toLocaleLowerCase().indexOf(term) > -1)
      matchingSession = matchingSession.map((session:any) => {session.eventId = event.id
         return session})
